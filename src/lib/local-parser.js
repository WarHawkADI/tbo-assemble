...
exports.extractTextWithOCRFallback = extractTextWithOCRFallback;
exports.validateContractText = validateContractText;
exports.validateInviteText = validateInviteText;
exports.parseTableRows = parseTableRows;
exports.extractRooms = extractRooms;
...
function normalizeText(raw) {
    ...
    text = text.replace(/\b([A-Za-z])( [A-Za-z]){2,}\b/g, (match) => match.replace(/ /g, ""));
    return text;
}
// ═══════════════════════════════════════════════════════════════════════════════
 ...
 */
function extractJPEGsFromPDFBuffer(buffer) {
    const jpegs = [];
 ...
 */
async function extractTextWithOCRFallback(buffer) {
    let text = "";
...
function validateContractText(text) {
    if (!text || text.trim().length < 20) {
        return {
            ...
            confidence: 0,
            matchedKeywords: [],
            error: "Could not extract readable text from this file. The file may be corrupted, password-protected, or contain only scanned images with no recognizable text.",
        };
    }
...
function validateInviteText(text) {
    ...
    if (matched.length < 2) {
        return {
            ...
            confidence,
            matchedKeywords: matched,
            error: "This document does not appear to be an event invitation. No event-related terms were found. Please upload a valid wedding invitation, conference flyer, or event document.",
        };
    }
    return { isValid: true, confidence, matchedKeywords: matched };
}
...
function extractLabeledFields(text) {
    ...
    const normalizeHeaderKey = (key) => {
        ...
        if (/^check[\s-]*in\s*date$/.test(k))
            return "check-in";
        if (/^check[\s-]*out\s*date$/.test(k))
            return "check-out";
        if (/^arrival\s*date$/.test(k))
            return "arrival";
        if (/^departure\s*date$/.test(k))
            return "departure";
        return k;
    };
...
function parseTableRows(text) {
    ...
    const headerPattern = /(?:description|item|particular|service|room\s*type|category|accommodation)\s+.*?(?:qty|quantity|rate|amount|total|price|charge|cost|number|count|no\.?\s*of)/i;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^(?:terms|conditions|notes|signatures?|authorized|disclaimer|important|policy|clause|payment\s*terms)/i.test(line)) {
            inTable = false;
            continue;
        }
        ...
        const sanitizedLine = line.replace(/\(\s*\d+\s+[A-Za-z][A-Za-z .]+\)/g, (m) => " ".repeat(m.length));
        ...
        if (numberTokens.length === 1) {
            ...
            for (let qtyLen = 1; qtyLen <= 5 && qtyLen < rawNum.length - 6; qtyLen++) {
                ...
                if (qtyStr.startsWith("0"))
                    continue; // no leading zeros in qty
            ...
            if (found) {
                numberTokens = [
                    { 1: String(bestQty), index: numberTokens[0].index },
                    { 1: String(bestRate), index: numberTokens[0].index + 1 },
                    { 1: String(bestTotal), index: numberTokens[0].index + 2 },
                ];
            }
            else {
                ...
                if (concatB) {
                    ...
                    if (n1 > 0 && n2 > 0) {
                        numberTokens = [
                            { 1: String(n1), index: numberTokens[0].index },
                            { 1: String(n2), index: numberTokens[0].index + 1 },
                        ];
                    }
                }
            }
        ...
        const isTotal = /^(?:total|sub\s*total|grand\s*total|net\s*total|gross\s*total)/i.test(description);
        if (isTotal) {
            inTable = false;
            continue;
        }
    ...
    if (rows.length === 0) {
        for (const line of lines) {
            ...
            const tableRowMatch = trimmed.match(/^([A-Za-z][A-Za-z\s/&()\-]{2,60}?)\s+([\d,]+(?:\.\d{1,2})?)\s+([\d,]+(?:\.\d{1,2})?)\s+([\d,]+(?:\.\d{1,2})?)\s*$/);
            if (tableRowMatch) {
            ...
            if (rows.length === 0) {
                const twoNumMatch = trimmed.match(/^([A-Za-z][A-Za-z\s/&()\-]{2,60}?)\s+([\d,]+(?:\.\d{1,2})?)\s+([\d,]+(?:\.\d{1,2})?)\s*$/);
                if (twoNumMatch) {
 ...
 */
function interpretTableRow(numbers) {
    if (numbers.length === 0)
        return { rate: 0, quantity: 1 };
    if (numbers.length === 1) {
    ...
    if (smallest < 500 && smallest > 0) {
        ...
        const rateCandidate = numbers.find((n) => n !== smallest &&
            n !== largest &&
        ...
        const middleNumbers = numbers.filter((n) => n !== smallest && n !== largest);
...
function extractVenue(text, fields) {
    ...
    const hotelSuffixPattern = /(?:Hotel|Resort|Palace|Inn|Lodge|Retreat|Spa|Suites|Mansion|Manor|Club|Tower|Centre|Center|Plaza|Residency|Regency|Continental|International)(?:\s*&\s*[A-Za-z]+)?$/i;
    for (let i = 0; i < candidates.length; i++) {
        const name = candidates[i];
        if (hotelSuffixPattern.test(name))
            continue; // already ends with hotel word
    ...
    for (const pattern of labelPatterns) {
        ...
        while ((match = pattern.exec(text)) !== null) {
            ...
            if (name.length >= 3 && !name.includes("http"))
                candidates.push(name);
        }
    }
    // Strategy 3: "Between [Party A] and [Hotel Name]" — common contract opening
    const betweenMatch = text.match(/between\s+.{3,80}?\s+and\s+([A-Z][A-Za-z\s&']+?(?:Hotel|Resort|Palace|Inn|Lodge|Retreat|Spa|Suites|Mansion|Manor|Club|Tower|Centre|Center|Plaza|Residency|Regency|Continental|International)(?:\s*&\s*[A-Za-z]+)?)/i);
    if (betweenMatch?.[1]) {
    ...
    for (const pattern of fullNamePatterns) {
        ...
        while ((match = pattern.exec(text)) !== null) {
            ...
            name = name
                .replace(/\s+(?:rooms?|is|are|was|were|has|have|had|will|would|shall|should|may|can|could|must|also|subject|under|provides?|offers?|includes?|features?|situated|located|near|with|from|this|that|which|where|when|being|been|for\s+(?:the|a)|at\s+(?:the|a)|of\s+(?:the|a))\b.*/i, "")
                .trim();
            if (name.length >= 3 &&
                !name.includes("/") &&
                !name.includes("\\") &&
    ...
    for (const hw of hotelWords) {
        ...
        while ((idx = lower.indexOf(hw, searchFrom)) !== -1) {
            ...
            const capPhrase = window.match(/([A-Z][A-Za-z']+(?:[^\S\n]+(?:[A-Z&][A-Za-z']*|the|of|de|le|by|at)){1,6})/);
    ...
    const scored = unique.map((name) => {
        ...
        if (hotelSuffixes.test(name))
            score += 50;
        // Contains a branded chain name → boost
        if (/Taj|Oberoi|ITC|Marriott|Hilton|Hyatt|Radisson|Sheraton|Westin|JW|Ritz|Four\s*Seasons|Le\s*Meridien|Leela|Lalit|Trident|Vivanta|Sofitel|Pullman|Kempinski|St\.?\s*Regis/i.test(name))
            score += 30;
 ...
 */
function extractLocation(text, fields) {
     ...
     */
    const extractCityFromAddress = (s) => {
        if (!/^\d+\s+/.test(s))
            return null; // must start with street number
        ...
        if (/^\d/.test(cityCandidate) || cityCandidate.length > 40)
            return null;
    ...
    for (const pattern of labelPatterns) {
        ...
        if (match?.[1]) {
            ...
            if (cityFromAddr2)
                return cityFromAddr2;
            // Skip plain street addresses (e.g. "456 Maple Street") – no city component
            if (!isBareStreetAddress(candidate))
                return candidate;
        }
    }
    // Strategy 3: "in [City]" or "at [City]" patterns
    const inAtMatch = text.match(/(?:\bin\b|\bat\b)\s+([A-Z][a-z]+(?:,\s*[A-Z][A-Za-z\s]+)?)/);
 ...
 */
function extractDates(text, fields) {
    const months = MONTHS;
    ...
    if (!checkIn) {
        const checkInRegex = new RegExp(`(?:check[\\s-]*in|arrival|start)\\s*(?:date)?\\s*[:–—-]?\\s*\\n?\\s*${dateCapture}`, "i");
    ...
    if (!checkOut) {
        const checkOutRegex = new RegExp(`(?:check[\\s-]*out|departure|end)\\s*(?:date)?\\s*[:–—-]?\\s*\\n?\\s*${dateCapture}`, "i");
    ...
    const rangePattern1 = new RegExp(`(\\d{1,2})\\s*[-–—to]+\\s*(\\d{1,2})\\s+(${monthNames})\\s+(\\d{4})`, "gi");
    const rangePattern2 = new RegExp(`(${monthNames})\\s+(\\d{1,2})\\s*[-–—to]+\\s*(\\d{1,2}),?\\s*(\\d{4})`, "gi");
    ...
    // DD Month YYYY
    const longDatePattern = new RegExp(`(\\d{1,2})\\s+(${monthNames})\\s+(\\d{4})`, "gi");
    while ((m = longDatePattern.exec(text)) !== null) {
        const day = m[1].padStart(2, "0");
        const month = months[m[2].toLowerCase()];
        if (month)
            parsed.push(`${m[3]}-${month}-${day}`);
    }
    // Month DD, YYYY
    const longDatePattern2 = new RegExp(`(${monthNames})\\s+(\\d{1,2}),?\\s+(\\d{4})`, "gi");
...
function extractContractNo(text, fields) {
...
function extractIssueDate(text, fields) {
    ...
    if (m) {
        const p = parseDateFromString(m[1].trim(), MONTHS);
        if (p)
            return p;
    }
    return "";
...
function extractValidUntil(text, fields) {
...
function extractNights(text, fields) {
...
function extractGroupCode(text, fields) {
...
function extractPaymentTerms(text, fields) {
    for (const k of ["payment terms", "payment", "advance payment", "billing terms"]) {
        if (fields[k])
            return fields[k].trim().substring(0, 200);
    }
...
function extractEarlyCheckIn(text) {
...
function extractLateCheckOut(text) {
...
function extractAgentContact(text, fields) {
    const result = {};
    ...
    const agentLine = text.match(/(?:coordinating\s*agent|travel\s*agent|booking\s*agent|agent|contact)\s*\n?\s*([A-Z][a-z]+(?: [A-Z][a-z]+)+ [·\-] [^\n]{5,120})/i) || text.match(/(?:coordinating\s*agent|travel\s*agent|booking\s*agent|agent)\s*\n?\s*([^\n]{5,120})/i);
    if (agentLine) {
 ...
 */
function extractRooms(text, tableRows) {
    const rooms = [];
    const seen = new Set();
    ...
    if (tableRows.length > 0) {
        for (const row of tableRows) {
            ...
            const isRoom = /room|suite|villa|cottage|studio|deluxe|standard|premium|executive|royal|superior|twin|double|single|king|queen|club|penthouse|accommodation/i.test(desc);
            if (isRoom) {
    ...
    if (rooms.length === 0) {
        ...
        for (const roomType of roomTypeWords) {
            ...
            if (seen.has(normalizedType.toLowerCase()))
                continue;
            ...
            const line = text.substring(lineStart >= 0 ? lineStart + 1 : 0, lineEnd >= 0 ? lineEnd : text.length);
            ...
            let context = text.substring(idx, Math.min(text.length, idx + 800));
            ...
            if (lineNumbers.length >= 2) {
                ...
                for (const rp of ratePatterns) {
                    ...
                    if (rateMatch) {
                        const val = parseInt((rateMatch[1] || rateMatch[2] || "0").replace(/,/g, ""));
                ...
                if (quantity === 0) {
                    const qtyBefore = beforeContext.match(/(\d+)\s*(?:rooms?|units?|nos\.?|x|×)\s*$/i);
                    if (qtyBefore)
            ...
            // Extract floor
            let floor = "";
            const floorMatch = context.match(/(?:floor|levels?|level|storey|storeys?)s?\s*[:=]?\s*([A-Za-z0-9\-–]+)/i);
            if (floorMatch) {
                if (!/^s$/i.test(floorMatch[1]))
                    floor = floorMatch[1];
            }
            // Extract wing
            let wing = "";
            const wingMatch = context.match(/(?:wing|tower|block|building)\s*[:=]?\s*([A-Za-z\s]+?)(?:\s*[,.\n|]|$)/i);
            if (wingMatch)
            ...
            let hotelName = "";
            const hotelMatch = (beforeContext + context).match(/(?:at|hotel|property)\s*[:=]?\s*([A-Z][A-Za-z\s&']+(?:Hotel|Resort|Palace|Spa|Suites)?)/);
            if (hotelMatch)
    ...
    if (rooms.length === 0) {
        const tableHeaderIdx = text.search(/room\s*type|category|accommodation\s*type/i);
        if (tableHeaderIdx >= 0) {
            ...
            for (const line of lines) {
                if (line.trim().length < 5)
                    continue;
                const lineMatch = line.match(/([A-Za-z][A-Za-z\s/&-]{2,40}?)\s+.*?([\d,]{3,})/);
                if (lineMatch) {
                    const name = lineMatch[1].trim();
                    const rate = parseInt(lineMatch[2].replace(/,/g, ""));
                    const qtyInLine = line.match(/(\d+)\s*(?:rooms?|units?|nos?\.?|keys|pax)/i);
 ...
 */
function extractAddOns(text, tableRows) {
    const addOns = [];
    const seen = new Set();
    ...
    const isJunkName = (n) => {
        ...
        if (/^\d+\/\d+/.test(n))
            return true; // date fragment like "2/22/"
        if (n.includes("\\") || n.includes("%20"))
            return true; // encoded path
        if (/^[^A-Za-z]{4,}/.test(n))
            return true; // starts with 4+ non-letters
        return false;
    };
    ...
    for (const row of tableRows) {
        ...
        if (/^(?:deluxe|standard|premium|executive|royal|superior|twin|double|single|king|queen|club|penthouse)\s+(?:room|suite|villa|cottage|studio)/i.test(desc) ||
            /\broom\b.*\b(?:deluxe|suite|standard|premium|executive|superior)\b/i.test(desc))
            continue;
        // Also skip if it ONLY says "Deluxe Rooms", "Standard Rooms" etc.
        if (/^(?:deluxe|standard|premium|executive|royal|superior)\s+rooms?$/i.test(desc))
            continue;
        // Check if it's a service/add-on
        const isService = /banquet|hall|catering|audio|visual|setup|rental|decoration|photography|dj|music|entertainment|transfer|shuttle|airport|valet|parking|flowers?|stage|lighting|sound|projector|screen|podium|ceremony|food|beverage|drinks?|bar|lounge|menu|buffet|pax|breakfast|lunch|dinner|spa|wifi|internet|gym|pool|laundry|concierge/i.test(desc);
        // If it has numbers and isn't a pure room type, treat it as a service
        if ((isService || row.numbers.length >= 2) &&
            !seen.has(desc.toLowerCase())) {
            // Double check it's not a room
            const isDefinitelyRoom = /^(?:deluxe|standard|premium|executive|royal|superior|family|luxury|classic|junior|grand|honeymoon|penthouse|villa|cottage|studio|twin|double|single|king|queen|club|garden|ocean|lake|mountain|valley|pool|city)\s/i.test(desc);
            if (isDefinitelyRoom)
                continue;
    ...
    for (const { pattern, name } of addOnPatterns) {
        if (seen.has(name.toLowerCase()) || seen.has(name))
            continue;
        // Also skip if the strategy-1 table rows already captured a fuller version of this item
        if (tableAddOnNames.some((n) => n.includes(name.toLowerCase())))
            continue;
        ...
        if (match) {
            ...
            const currentLine = lower.substring(lineStart >= 0 ? lineStart : 0, lineEnd >= 0 ? lineEnd : lower.length);
            ...
            const twoLines = lower.substring(lineStart >= 0 ? lineStart : 0, nextLineEnd >= 0 ? nextLineEnd : lower.length);
            // Check complimentary on current + next line (PDF text often splits label and status onto separate lines)
            const isIncluded = /complimentary|included|free|no\s*charge|inclusive|gratis|at no cost/i.test(twoLines)
                && !/[₹$]\s*[\d,]{3,}|(?:rs\.?|inr)\s*[\d,]{3,}/i.test(twoLines);
            let price = 0;
            if (!isIncluded) {
                // Look for price AFTER the keyword (not before) to avoid picking up adjacent add-on prices
                const afterKeyword = lower.substring(idx + match[0].length, Math.min(lower.length, idx + match[0].length + 80));
                const afterPriceMatch = afterKeyword.match(/[₹$]\s*([\d,]+)|(?:rs\.?|inr)\s*([\d,]+)/i);
                if (afterPriceMatch) {
 ...
 */
function extractAttritionRules(text) {
    const rules = [];
    // Focus on the attrition/cancellation section if it exists
    const attritionIdx = text
        .toLowerCase()
        .search(/attrition|release\s*schedule|cancellation\s*policy|cancellation\s*clause|penalty|cut[\s-]*off|terms\s*(?:&|and)\s*conditions/);
    ...
    const dateThenPercent = /((?:january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|oct|nov|dec)\s+\d{1,2},?\s+\d{4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4}|\d{4}-\d{2}-\d{2})\s*(?:[^\n]*?)(\d+)\s*%\s*(?:release|cancel|reduction|unsold|attrition)/gi;
    ...
    if (rules.length === 0) {
        const percentThenDate = /(\d+)\s*%\s*(?:.*?)((?:january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|oct|nov|dec)\s+\d{1,2},?\s+\d{4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4}|\d{4}-\d{2}-\d{2})/gi;
    ...
    if (rules.length === 0) {
        const daysBeforePattern = /(\d+)\s*days?\s*(?:before|prior|ahead|in\s*advance).*?(\d+)\s*%/gi;
    ...
    if (rules.length === 0) {
        const percentDaysPattern = /(\d+)\s*%\s*(?:.*?)(\d+)\s*days?\s*(?:before|prior|ahead|in\s*advance)/gi;
    ...
    if (rules.length === 0) {
        const withinDaysPattern = /(?:cancellation|cancel)\s*(?:within|inside|less\s*than)\s*(\d+)\s*days?\s*(?:of|before|from)?\s*(?:the\s*)?(?:event|check[\s-]*in|arrival|function)?[^.]*?(\d+)\s*%\s*(?:charge|penalty|fee|of\s*(?:the\s*)?total|cancellation)/gi;
    ...
    if (rules.length === 0) {
        const incurPattern = /(?:cancellation\s*)?(?:within|inside)\s*(\d+)\s*days?[^.]*?(?:will\s*)?(?:incur|attract|result\s*in|lead\s*to|be\s*subject\s*to)\s*(\d+)\s*%/gi;
    ...
    if (rules.length === 0) {
        const penaltyPattern = /(?:penalty|charge|fee)\s*(?:of|:)?\s*(\d+)\s*%/gi;
    ...
    if (rules.length === 0) {
        const percentPenaltyPattern = /(\d+)\s*%\s*(?:cancellation\s*)?(?:penalty|charge|fee)/gi;
 ...
 */
function extractClientName(text, fields) {
    ...
    const betweenMatch = text.match(/between\s+([A-Z][A-Za-z\s&'.]+?)\s+(?:and|&)\s+(?:[A-Z].*?(?:Hotel|Resort|Palace|Inn|Lodge|Retreat|Spa))/i);
    if (betweenMatch?.[1]) {
    ...
    const eventNameFromText = fields["event name"] ||
        fields["function name"] ||
        text.match(/([A-Z][a-z]+(?:\s*[-&]\s*[A-Z][a-z]+)+)\s+Wedding/)?.[1] || "";
    if (eventNameFromText) {
        const familyM = eventNameFromText.match(/([A-Z][a-z]+(?:\s*[-&]\s*[A-Z][a-z]+)*)\s+(?:Wedding|Marriage|Shaadi)/i);
        if (familyM)
            return familyM[1].trim() + " Families";
    }
    // Strategy 5: "X Families" or "X and Y Family"
    const familyLineMatch = text.match(/([A-Z][a-z]+(?:\s*(?:&|and)\s*[A-Z][a-z]+)*)\s+Famil(?:y|ies)/i);
    if (familyLineMatch)
        return familyLineMatch[1].trim() + " Families";
    return "";
}
// ═══════════════════════════════════════════════════════════════════════════════
...
function extractEventName(text, eventType, fields) {
    // Strategy 1: Labeled fields
    const nameFieldKeys = [
    ...
    if (eventType === "wedding") {
        const familyPattern = text.match(/([A-Z][a-z]+)\s*(?:&|and|-|weds|❤)\s*([A-Z][a-z]+)/);
        if (familyPattern) {
...
// ═══════════════════════════════════════════════════════════════════════════════
async function extractColorsFromImage(buffer) {
    try {
        const sharp = (await Promise.resolve().then(() => __importStar(require("sharp")))).default;
        const { dominant } = await sharp(buffer).stats();
        const primary = rgbToHex(dominant.r, dominant.g, dominant.b);
        const secondary = rgbToHex(Math.min(255, dominant.r + 100), Math.min(255, dominant.g + 100), Math.min(255, dominant.b + 100));
        const accent = rgbToHex(Math.min(255, Math.abs(dominant.r - 40) + 80), Math.min(255, Math.abs(dominant.g - 20) + 60), Math.min(255, Math.abs(dominant.b + 30)));
 ...
 */
async function parseContractLocally(buffer, mimeType) {
    let text = "";
    let usedOCR = false;
    if (mimeType.includes("pdf")) {
        ...
        try {
            ...
            return {
                success: false,
                error: "Failed to extract text from this PDF. The file may be corrupted, password-protected, or contain only scanned images.",
            };
        }
    }
        ...
        try {
            ...
            return {
                success: false,
                error: "Failed to extract text from this image. Please upload a clearer image or a text-based PDF.",
            };
        }
    }
    else {
        return {
            success: false,
            error: "Unsupported file type. Please upload a PDF or image file (PNG, JPEG, WebP).",
        };
    }
    // Validate content
 ...
 * Returns structured data or a validation error.
 */
async function parseInviteLocally(buffer, mimeType) {
    let text = "";
    ...
    if (text) {
        ...
        if (hexColors && hexColors.length >= 2)
            colors.secondary = hexColors[1];
        if (hexColors && hexColors.length >= 3)
            colors.accent = hexColors[2];
        if (!hexColors || hexColors.length === 0) {
            ...
            const foundColors = [];
            const sortedNames = Object.keys(namedColorMap).sort((a, b) => b.length - a.length);
    ...
    if (text) {
        ...
        if (iCheckIn)
            data.checkIn = iCheckIn;
        if (iCheckOut)
            data.checkOut = iCheckOut;
