...
function extractNights(text, fields) {
    ...
    const m = text.match(/(\d+)\s*nights?\s*(?:\/|\s)\s*\d+\s*days?/i);
    if (m)
        return parseInt(m[1]);
...
function extractTotalAmount(text, fields) {
    ...
    const m = text.match(/(?:total\s*(?:block\s*)?(?:amount|value|price|cost|invoice|charges?))\s*[:=]?\s*[₹$€£]?\s*([\d,]+)/i);
    if (m)
...
function extractTaxInfo(text) {
    const m = text.match(/((?:gst|vat|tax|cess|service\s*charge)[^.\n]{0,120})/i);
    if (m)
...
function extractGSTIN(text, fields) {
    ...
    // Standard 15-char GSTIN: 2 digits + 5 letters + 4 digits + 1 letter + 1 digit + Z + 1 alphanumeric
    const m = text.match(/(?:gstin|gst\s*(?:no\.?|number|in))\s*[:=]?\s*([0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][0-9][Z][A-Z0-9])/i);
    if (m)
        return m[1].toUpperCase();
    // Loose fallback for common malformed GST strings
    const m2 = text.match(/(?:gstin)\s*[:=]?\s*([A-Z0-9]{15})/i);
    if (m2)
...
function extractHotelContact(text, fields) {
    ...
    // Look for hotel representative / DOSm block in text
    const hotelRepMatch = text.match(/(?:director\s*of\s*sales|hotel\s*representative|sales\s*manager|gm|general\s*manager|reservations\s*manager)\s*\n?([A-Z][a-z]+(?: [A-Z][a-z]+)+)/i);
    if (hotelRepMatch)
...
function extractAgentContact(text, fields) {
    ...
    const agentLine = text.match(/(?:coordinating\s*agent|travel\s*agent|booking\s*agent|agent)\s*\n?\s*([^\n]{5,120})/i);
    if (agentLine) {
    ...
    if (rooms.length === 0) {
        ...
        for (const roomType of roomTypeWords) {
            ...
            if (!wing || /^Wing$/i.test(wing)) {
                const ctxWing = context.match(/([A-Z][a-z]+)\s+Wing/i);
                if (ctxWing)
                    wing = ctxWing[1];
            }
...
function extractAttritionRules(text) {
    ...
    if (rules.length === 0) {
        const advancePattern = /(\d+)\s*%\s*(?:advance|upfront|down)\s*(?:payment|deposit)/gi;
